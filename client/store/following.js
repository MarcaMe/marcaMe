...
export const fetchFollowing = userId => dispatch => {
  axios
    .get(`/api/relationship/following/${userId}`)
    .then(res => res.data)
    .then(followingArr => dispatch(getFollowing(followingArr)))
    .catch(err => console.error(err));
};

export const removeFollowing = (userId, followingId) => dispatch => {
  axios({
    method: 'DELETE',
...
export const addOneFollowing = (followingId, userId) => dispatch => {
  axios
    .post(`/api/relationship/following/${userId}`, { followingId: followingId })
    .then(res => res.data)
